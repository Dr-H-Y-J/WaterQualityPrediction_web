...
const fetchData = async () => {
  try {
    ...
    if (rolesRes.data.success) {
      // 处理角色数据，确保时间格式正确
      groups.value = rolesRes.data.data.map(role => ({
        ...role,
        userIds: Array.isArray(role.userIds) ? role.userIds : [],
        permissions: Array.isArray(role.permissions) ? role.permissions : [],
        createTime: role.createTime || role.created_at || '',
        updateTime: role.updateTime || role.updated_at || ''
      }))
    }

    if (usersRes.data.success) {
      // 处理用户数据，确保字段格式正确
      users.value = usersRes.data.data.map(user => ({
        ...user,
        id: String(user.id), // 确保ID是字符串
        role: user.role || user.user_role || 'user', // 确保角色字段存在
        username: user.username || `用户${user.id}`, // 确保用户名存在
        email: user.email || '' // 确保邮箱字段存在
      }))
    }
    ...
    ElMessage.error('数据加载失败: ' + (error.response?.data?.message || error.message || '网络错误'))
  } finally {
    loading.value = false
  }
}
...
const getGroupUsers = (userIds) => {
  ...
  return users.value.filter((user) => userIds.includes(String(user.id)))
}
...
const saveGroup = async () => {
  ...
  try {
    ...
    if (dialogType.value === 'create') {
      // 创建新组别
      response = await axios.post('/api/roles', {
      ...
      // 编辑组别
      response = await axios.put(`/api/roles/${currentGroup.value.id}`, {
        name: groupForm.value.name,
        description: groupForm.value.description,
        permissions: groupForm.value.permissions,
        userIds: groupForm.value.userIds
...
const removeUserFromGroup = async (group, userId) => {
  try {
    const updatedUserIds = group.userIds.filter(id => String(id) !== String(userId))
    ...
    ElMessage.error('移除失败: ' + (error.response?.data?.message || error.message || '网络错误'))
  }
}
      ...
      <el-card v-for="group in filteredGroups" :key="group.id" class="group-card" shadow="hover">
        ...
        <div class="card-content">
          ...
          <div class="users-section">
            <h4>
              <el-icon><Users /></el-icon> 组别成员 ({{ group.userIds ? group.userIds.length : 0 }}人)
            </h4>
            <div class="users-list">
              <div v-for="user in getGroupUsers(group.userIds)" :key="user.id" class="user-item">
                ...
                <el-tag size="small" :type="(user.role || '').includes('admin') ? 'danger' : 'default'">
                  {{ user.role || 'user' }}
                </el-tag>
                <el-button
                  type="danger"
          ...
          <div class="group-meta">
            <el-text size="small" type="info"> 创建时间: {{ group.createTime || '未知' }} </el-text>
            <el-text size="small" type="info"> 更新时间: {{ group.updateTime || '未知' }} </el-text>
          </div>
        </div>
      </el-card>
    </div>
